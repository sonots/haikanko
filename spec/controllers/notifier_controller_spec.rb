...
shared_context 'edit_notifier_controller' do |button = 'Create', params = {}|
  before do
    ...
    fill_in 'notifier[roles]',    :with => params["roles"]    if params["roles"]
    ...
    fill_in 'notifier[irc_label]',   :with => params["irc_label"]   if params["irc_label"]
    fill_in 'notifier[email_label]', :with => params["email_label"] if params["email_label"]
    click_button button
  end
end
...
shared_examples_for "show_notifier_controller" do
  ...
  it { find_field("notifier[roles]").should be_visible }
  it { find_field("notifier[log_path]").should be_visible }
  it { find_field("notifier[regexp]").should be_visible }
  it { find_field("notifier[irc_label]").should be_visible }
  it { find_field("notifier[email_label]").should be_visible }
end
